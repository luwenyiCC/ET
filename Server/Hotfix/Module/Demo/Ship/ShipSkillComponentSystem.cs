...
namespace ETHotfix
{
    public static class ShipSkillComponentHelper
    {
        public static async ETTask MoveAsync(this ShipSkillComponent self, List<Vector3> path)
        {
            if (path.Count == 0)
            {
                return;
        ...
        // 从index找接下来3个点，广播
        public static void BroadcastPath(this ShipSkillComponent self, List<Vector3> path, int index, int offset)
        {
            Ship unit = self.GetParent<Ship>();
